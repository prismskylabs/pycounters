...
class MuninTests(unittest.TestCase):
    ...
    def test_munin_zero_values(self):
        cfg = self.make_basic_cfg()
        cfg[0]["global"]["pyc_ignore_zero_value"]=True
        plugin = self.create_plugin(cfg)
        ...
        try:
            test1.report_event("test1", "value", 0)

            GLOBAL_REPORTING_CONTROLLER.report()

            plugin.output_data(cfg)
            self.assertEqual(self.get_last_plugin_output(), ["multigraph test" ])
    ...
    def test_munin_zero_values_disabled(self):
        cfg = self.make_basic_cfg()
        cfg[0]["global"]["pyc_ignore_zero_value"]=False
        plugin = self.create_plugin(cfg)
        ...
        try:
            ...
            plugin.output_data(cfg)
            self.assertEqual(self.get_last_plugin_output(), ["multigraph test", "test1.value 0" ])
